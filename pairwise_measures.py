...
class MultiClassPairwiseMeasures(object):
    ...
    def one_hot_pred(self):
        return one_hot_encode(self.pred, np.max(self.list_values) + 1)

    def one_hot_ref(self):
        return one_hot_encode(self.ref, np.max(self.list_values) + 1)
    ...
    def expectation_matrix(self):
        ...
        return np.matmul(pred_numb[..., None], ref_numb[None]) \
    ...
    def weighted_cohens_kappa(self):
        ...
        if "weights" in self.dict_args.keys():
            ...
            weights = np.ones([len(self.list_values), len(self.list_values)]) \
...
class BinaryPairwiseMeasures(object):
    def __init__(
        ...
        self.pred = np.where(pred, 1, 0)  # binarise
        self.ref = np.where(ref, 1, 0)  # binarise
    ...
    def __union_map(self):
        """
        This function calculates the union map between predicted and
        reference image
        :return: union map
        """
        return np.asarray((self.ref + self.pred) > 0.5, dtype=np.float32)
    ...
    def __intersection_map(self):
        """
        This function calculates the intersection between predicted and
        reference image
    ...
    def precision(self):
        if self.n_pos_pred() == 0:
            if self.n_pos_ref() == 0:
                warnings.warn("ref and prediction empty precision not defined")
                return np.nan
            else:
                warnings.warn("prediction empty, precision not defined but set to 0")
                return 0
    ...
    def expected_matching_ck(self):
        # https://en.wikipedia.org/wiki/Cohen%27s_kappa#Definition (just two raters)        
        list_values = np.unique(self.ref)
        p_e = 0
        for val in list_values:
            p_er = np.sum(self.ref == val) / np.prod(self.ref.shape)
            p_es = np.sum(self.pred == val) / np.prod(self.pred.shape)
            p_e += p_es * p_er
        return p_e
    ...
    def fbeta(self):
        if "beta" in self.dict_args.keys():
            ...
            beta = 1
        numerator = (
            (1 + np.square(beta)) * self.precision() * self.sensitivity()
        )
        denominator = (
            np.square(beta) * self.precision() + self.sensitivity()
        )
    ...
    def intersection_over_reference(self):
        """
        This function returns the intersection over reference ratio
        :return:
        """
    ...
    def com_dist(self):
        ...
        if self.flag_empty:
            ...
            com_ref = self.center_of_mass_ref()
            com_pred = self.center_of_mass_pred()
    ...
    def center_of_mass_ref(self):
        """
        This function calculates the centre of mass of the reference image
        :return:
        """
        return compute_center_of_mass(self.ref)

    def center_of_mass_pred(self):
        """
        This functions provides the centre of mass of the predicted image
        :return:
        """
        if self.flag_empty:
            return -1
        else:
            return compute_center_of_mass(self.pred)

    def vol_diff(self):
        """
        This function calculates the ratio of difference in volume between
        the reference and predicted images.
        :return: vol_diff
        """
        return np.abs(self.n_pos_ref() - self.n_pos_pred()) / self.n_pos_ref()

    def skeletonize_ref(self):
        return compute_skeleton(self.ref)

    def skeletonize_pred(self):
        return compute_skeleton(self.pred)

    def topology_precision(self):
        skeleton_pred = self.skeletonize_pred()
    ...
    def border_distance(self):
        ...
        distance_ref = distance_transform_edt(1 - border_ref, sampling=self.pixdim)
        distance_pred = distance_transform_edt(1 - border_pred, sampling=self.pixdim)
        distance_pred = border_ref * distance_pred
        distance_ref = border_pred * distance_ref
        return distance_ref, distance_pred, border_ref, border_pred
    ...
    def normalised_surface_distance(self):
        ...
        reg_ref = np.where(dist_ref <= tau, 1, 0)
        reg_pred = np.where(dist_pred <= tau, 1, 0)
    ...
    def measured_distance(self, perc=95):
        ...
        average_distance = \
            (np.sum(ref_border_dist) + np.sum(pred_border_dist)) \
            / (np.sum(pred_border) + np.sum(ref_border))
        masd = 0.5 * (
        ...
        hausdorff_distance_95 = np.max(
            [
                np.percentile(ref_border_dist[ref_border_dist > 0], q=perc),
                np.percentile(pred_border_dist[pred_border_dist > 0], q=perc),
            ]
        )
