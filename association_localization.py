...
class AssociationMapping(object):
    def __init__(self, pred_loc, ref_loc, pred_prob, localization='iou',thresh=0.5, association='Greedy IoU'):
        ...
        if df_ambiguous_ref is None or df_ambiguous_ref.shape[0] == 0 and df_ambiguous_seg.shape[0] == 0:
            ...
            if self.association == 'Hungarian':
                ...
                for (r,c) in zip(row,col):
                    df_tmp = df_matching[df_matching['seg']==list_valid[r] &
                                         (df_matching['ref']==c)]
                    list_matching.append(df_tmp)
                df_ordered2 = pd.concat(list_matching)
            elif self.association == 'Greedy IoU':
                if self.localization == 'com':
                ...
                df_ordered = df_matching.sort_values('pred_prob', ascending=False).drop_duplicates(
                    'pred')
                df_ordered2 = df_ordered.sort_values('pred_prob', ascending=False).drop_duplicates(
                    'ref')
