...
# plot the predicted and reference data
fig, axs = plt.subplots(1, 2)
axs[0].imshow(prediction, cmap='gray')
axs[0].set_title('Predicted')
axs[1].imshow(reference, cmap='gray')
...
print(f'fbeta =  {bpm.fbeta()}\n')

print('Creation of process for multiple cases')
# multi label pairwise measures
mlpm = MLPM(list_pred,
            list_ref,
            list_prob,
            list_values=[1],
            measures_pcc=['fbeta','nsd'],
            per_case=True)

df_seg, df_mt = mlpm.per_label_dict()
print(df_seg)

print("\nCreation of multi process with warning in choice of measure due to absence of probabilistic input")
mlpm = MLPM(list_pred,
            list_ref,
            list_prob,
            list_values=[1],
